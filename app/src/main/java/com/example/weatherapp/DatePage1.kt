...
class DatePage1 : Fragment() {
    ...
    override fun onCreateView(
        ...
        Log.d("aaa", "datePage1")
        val view = inflater.inflate(R.layout.page_date1, container, false)
        dateView = view.findViewById(R.id.dateView1)
        ok = true
        dateView?.setText(info)
        if (dateView != null) {
            Log.d("aaa", "dateView init")
        }
        Log.d("aaa", "DatePage1 finish")
        if (ok) {
            Log.d("aaa", "ok init finish")
        }
        return view
    }

    fun setPage(weatherInfo: WeatherResponse, pos: Int) {
        Log.d("aaa", "datePage1 setPage")
